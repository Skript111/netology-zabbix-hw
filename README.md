...
Для сбора данных о загрузке диска создан скрипт `/usr/local/bin/custom_disk_usage.py`.  
В конфигурационный файл агента добавлена строка:

Еще доп скриншот
![1](img/23.png)

Вроде бы я уже все скриншоты приложил. Не знаю что еще нужно добавить для зачета... Работает и уже хорошо
